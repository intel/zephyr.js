...
static void zjs_timer_call_function(struct zjs_callback *cb)
{
    ...
    if (jerry_value_has_error_flag(rval)) {
        PRINT("zjs_timer_call_function: error in js\n");
    }
    jerry_release_value(rval);
}

static jerry_value_t add_timer(uint32_t interval,
                               ...
                               bool repeat)
{
    ...
    tm->zjs_cb.js_callback = jerry_acquire_value(callback);
...
static bool delete_timer(jerry_value_t obj)
{
    ...
    for (struct zjs_timer_t **ptm = &zjs_timers; *ptm; ptm = &(*ptm)->next) {
        ...
        if (obj == tm->zjs_cb.js_callback) {
            jerry_release_value(tm->zjs_cb.js_callback);
                                                 ...
                                                 const jerry_length_t args_cnt)
{
    ...
    jerry_value_t tid = add_timer(interval, callback, true);
    if (!tid) {
