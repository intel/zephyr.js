...
Found:
            ...
            zjs_trigger_event(cur->client, "resourcefound", &args, 1, NULL, NULL);

            zjs_fulfill_promise(h->promise_obj, &args, 1);
                                        ...
                                        const jerry_length_t argc)
{
    ...
    if (zjs_ocf_start() < 0) {
        ...
        jerry_value_t err = make_ocf_error("NetworkError", "Error code from GET", NULL);
                                ...
                                const jerry_length_t argc)
{
    ...
    if (oc_init_put(resource->resource_path,
        ...
        if (!oc_do_put()) {
            ...
            jerry_value_t err = make_ocf_error("NetworkError", "PUT call failed", resource);
            zjs_reject_promise(promise, &err, 1);
            jerry_release_value(err);
        }
    } else {
        ERR_PRINT("error initializing PUT\n");
        jerry_value_t err = make_ocf_error("NetworkError", "PUT init failed", resource);
        zjs_reject_promise(promise, &err, 1);
        jerry_release_value(err);
    }
...
static void delete_finished(oc_client_response_t *data)
{
    if (data && data->user_data) {
        ...
        if (data->code == OC_STATUS_DELETED) {
            ...
            jerry_value_t err = make_ocf_error("NetworkError", "DELETE had error code", resource);
            zjs_reject_promise(h->promise_obj, &err, 1);
            jerry_release_value(err);
...
static void ocf_get_platform_info_handler(oc_client_response_t *data)
{
    if (data && data->user_data) {
        ...
        while (rep != NULL) {
            ...
            case STRING:
                if (strcmp(oc_string(rep->name), "mnmn") == 0) {
                    ...
                    zjs_obj_add_string(platform_info, oc_string(rep->value.string), "model");
                } else if (strcmp(oc_string(rep->name), "mndt") == 0) {
                    zjs_obj_add_string(platform_info, oc_string(rep->value.string), "manufacturerDate");
                } else if (strcmp(oc_string(rep->name), "mnpv") == 0) {
                    zjs_obj_add_string(platform_info, oc_string(rep->value.string), "platformVersion");
                } else if (strcmp(oc_string(rep->name), "mnos") == 0) {
                    zjs_obj_add_string(platform_info, oc_string(rep->value.string), "osVersion");
                } else if (strcmp(oc_string(rep->name), "mnfv") == 0) {
                    zjs_obj_add_string(platform_info, oc_string(rep->value.string), "firmwareVersion");
                }
                DBG_PRINT("%s\n", oc_string(rep->value.string));
                break;
            case STRING_ARRAY:
        ...
        jerry_release_value(platform_info);
    }
}
...
static void ocf_get_device_info_handler(oc_client_response_t *data)
{
    if (data && data->user_data) {
        ...
        jerry_release_value(device_info);
    }
}
